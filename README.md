...
#### ③ 被害エリアの指定機能
- 洪水または土砂災害が発生した場合に、任意の地点の情報をインプットすることでシミュレーションを行います。
- 洪水想定区域を利用する場合は、河川を選択て洪水被害を再現し、その中で損害を把握したいエリアを指定することができます。
...
| 入力データ | 洪水想定区域データ（FGDB形式）※1 | 3.0 | 洪水想定区域で浸水シミュレーションを行う場合に利用します。 |
...
損害額シミュレータのフォルダー構成のうち、設定ファイルの置き場などユーザが利用するフォルダのみ記載し、システムのみが利用するフォルダについては記載を省略します。
